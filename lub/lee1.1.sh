...
find --set-root /casper/$today/vmlinuz-`uname -r`\r
kernel /casper/$today/vmlinuz-`uname -r` boot=casper ro ignore_uuid\r
initrd /casper/$today/initrd.img-`uname -r`\r
\r
title Live Ubuntu Backup $today, Recovery Mode\r
find --set-root /casper/$today/vmlinuz-`uname -r`\r
kernel /casper/$today/vmlinuz-`uname -r` boot=casper ro single ignore_uuid\r
initrd /casper/$today/initrd.img-`uname -r`\r
\r
...
[ "`id -u`" != 0 ] && { echoen "Root privileges are required for running this program."; echocn "备份和恢复需要 root 权限。"; echousage;
exit 1; }
[ "$*" = -b ] && { dobackup; exit 0; }
[ "$*" = -r ] && { echoen "Be completed automatically restart it?(y/N)";echocn "完成后自动重启吗？(y/N)";read auto_reboot;dorestore;[ "$auto_reboot" == "y" ] && sudo reboot; exit 0; }
echousage
exit 1
